...
public class PostController {
	...
	public String getUploadPostForm(HttpSession session, HttpServletRequest request, HttpServletResponse response)
			throws IOException {
	...
	public String uploadPost(@RequestParam("description") String description,
			@RequestParam("locationName") String locationName, @RequestParam("latitude") String latitude,
			@RequestParam("longtitude") String longtitude, @RequestParam("taggedPeople") String taggedPeople,
			@RequestParam("tags") String tagNames, @RequestParam("categories") String categoryNames,
			@RequestParam("image1") MultipartFile image1, @RequestParam("image2") MultipartFile image2,
			@RequestParam("image3") MultipartFile image3, @RequestParam("video") MultipartFile video1,
			HttpSession session) {
		if (session.getAttribute("user") == null || session.getAttribute("logged").equals(false)) {
			return "login";
		}
		if ("".equals(description) && "".equals(locationName) && "".equals(latitude) && "".equals(longtitude)
				&& "".equals(taggedPeople) && "".equals(tagNames) && "".equals(categoryNames) && image1.getSize() == 0
				&& image2.getSize() == 0 && image3.getSize() == 0 && video1.getSize() == 0) {
	...
	private Location getLocation(String locationInput, String latitude, String longtitude)
			throws LocationException, SQLException, IOException {
	...
	private Multimedia readMultimedia(MultipartFile file, boolean isVideo) {
		...
		try {
			if (!file.isEmpty()) {
				File f = new File(
						WebInitializer.LOCATION + WebInitializer.MULTIMEDIA_LOCATION + File.separator + fileUrl);
				file.transferTo(f);
		...
		if (!"".equals(input)) {
			...
			for (int i = 0; i < splitCategories.length; i++) {
				...
				if (!"".equals(categoryName)) {
					...
					categoryName = categoryName.replace("[", "");
					HashMap<String, Category> applicationScopeCategories = (HashMap<String, Category>) servletContext
